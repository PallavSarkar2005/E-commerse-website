    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-950 via-purple-950 to-slate-950 p-4 overflow-hidden relative">
      ...
      >
        ...
        <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
          <div>
            <label className="block text-xs font-medium text-indigo-200 mb-1 uppercase tracking-wider">Email Address</label>
            <input 
              type="email" 
          ...
          <div className="flex items-center justify-between text-sm">
            <label className="flex items-center text-gray-300 cursor-pointer hover:text-white transition-colors">
              <input type="checkbox" className="mr-2 rounded bg-slate-800 border-slate-600 text-indigo-500 focus:ring-indigo-500/50" />
              Remember me
            </label>
            <a href="#" className="text-indigo-300 hover:text-indigo-200 transition-colors">Forgot Password?</a>
          </div>
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="w-full font-bold py-3.5 rounded-xl shadow-lg transition-all bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:shadow-indigo-500/25 border border-white/10"
          >
            Log In
          </motion.button>
        </form>
        <div className="mt-8 pt-6 border-t border-white/10 text-center">
          <p className="text-gray-300 text-sm">
            ...
            <Link to="/signup" className="ml-2 font-semibold hover:underline transition-colors text-amber-400 hover:text-amber-300">
              Sign Up
            </Link>
          </p>
          <div className="mt-4">
